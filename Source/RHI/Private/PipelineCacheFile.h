...
// An engine-owned header around a backend's opaque pipeline-cache blob.
//
// The drivers accept mismatched blobs silently (Vulkan) or report only
// some cases (D3D12), so "did my cache load?" answers yes for a file
// that contributed nothing. Our own header refuses the wrong engine,
// format version, backend or adapter before the driver sees it -- and
// reports the refusal. The payload hash catches the remaining case: a
// file meant for exactly this and damaged since.
